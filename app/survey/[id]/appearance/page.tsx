        ...
        <aside className="appearance-settings">
          ...
          <section>
            ...
            <div className="appearance-image-settings">
              ...
              <article><div><strong>问卷头图 · 桌面端</strong><small>建议 1440 × 360 px（4:1）；移动端未单独设置时沿用此图</small></div><button onClick={() => headerImageInputRef.current?.click()}>{config.headerImage ? "更换" : "上传"}</button>{config.headerImage && <button className="remove" onClick={() => update({ headerImage: "" })}>移除</button>}</article>
