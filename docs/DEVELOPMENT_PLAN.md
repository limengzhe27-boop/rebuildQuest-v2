...
> 当前交付物为独立评审与演示版本，部署在 Vercel 预览环境；尚未接入 JoyData 正式系统、真实账号权限或生产数据。
...
- [x] 定时结束、答卷上限与在途答卷宽限期
- [x] 玩家端问卷结束页与重新开启收集
- [x] 玩家端读取编辑器题目、题目逻辑与外观配置
- [x] 浏览器语言自动识别、链接语言参数与玩家手动切换
- [x] 玩家提交答卷并进入本机评审版答卷列表
- [ ] 业务评审反馈修订
- [ ] 模板导入、编辑器撤销/校验与真实文件导出
- [ ] JoyData 正式接口、权限与导航集成
